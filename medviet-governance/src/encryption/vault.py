...
class SimpleVault:
    ...
    def encrypt_data(self, plaintext: str) -> dict:
        ...
        nonce = os.urandom(12)
        ciphertext = aesgcm.encrypt(nonce, plaintext.encode(), None)  # Encrypt plaintext data
    ...
    def decrypt_data(self, encrypted_payload: dict) -> str:
        ...
        plaintext_dek = self.decrypt_dek(encrypted_dek)   # Decrypt DEK first
        nonce = ciphertext_with_nonce[:12]           # First 12 bytes is nonce
        ciphertext = ciphertext_with_nonce[12:]      # Remaining bytes is ciphertext
